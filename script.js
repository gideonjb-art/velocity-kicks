...
function getProductsGroupedByType(brandName){
  ...
  if (!window.shoesDB || window.shoesDB.length === 0){
    return {};
  }

  const brandProducts = window.shoesDB.filter(product => {
  ...
  brandProducts.forEach(product => {
    ...
    grouped[cleanType].push(product);
...
function renderBrandTypesPanel(brandName){
  ...
  const groupedProducts =
    getProductsGroupedByType(brandName);
...
window.handleProductClick = function(productId){
  ...
  if (typeof openProduct === 'function') {
    ...
    typeof openProductModal === 'function'
  ) {

    openProductModal(product);

  } else if (
    typeof showProductDetails === 'function'
  ) {
